...
func Create(name string, size int32, global bool) (*Memory, error) {
	m, err := create(name, size, global)
	if err != nil {
		return nil, err
	}
...
func Open(name string, size int32, global bool) (*Memory, error) {
	m, err := open(name, size, global)
	if err != nil {
		return nil, err
	}
	return &Memory{m, 0}, nil
